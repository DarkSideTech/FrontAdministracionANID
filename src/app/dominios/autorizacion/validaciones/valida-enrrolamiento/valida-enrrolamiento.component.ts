...
export class ValidaEnrrolamientoComponent implements OnInit  {
  ...
  columns = [
    ...
    { name: 'IdPersona' },
    { name: 'NombreUsuario' },
    { name: 'NombreADesplegar' },
    { name: 'Descripcion' },    
    { name: 'DireccionDeCorreoElectronico' },    
    { name: 'DireccionDeCorreoConfirmada' },
    { name: 'ClaveDeAcceso' },
    { name: 'NumeroDeTelefono' },
    { name: 'NumeroDeTelefonoConfirmado' },
    { name: 'PermiteSesionesDeVariosPerfiles' },
    { name: 'Activo' },
  ];
  booleanoType = [
    { id: '1', value: true },
...
desactivarRowSeleccionadas() {
  ...
  fetch(cb: any) {
    ...
    req.open('GET', 'assets/data/usuarios-tbl-data.json');
    req.onload = () => {
  ...
  activateRecordSuccess() {
    this.toastr.success('Se Valido el enrrolamiento del Usuario correctamente', '');
  }
  desactivateRecordSuccess() {
    this.toastr.success('Se Rechazo el enrrolamiento del Usuario', '');
  }
}
